...
import { addIngredientList, setIngredientViewList } from '@/services/ingredientList/slice';
...
const IngredientList = () => {
  ...
  const list = useAppSelector((state) => state.ingredientList.ingredientViewList);
  ...
  const addOnClick = () => {
    const duplicationCheck = ingredientList.ingredientList.filter((item) => item.name === addIngredientInput);
  ...
  useEffect(() => {
    switch (categoryValue) {
      case 'all':
        ...
        break;
      case 'meat':
        dispatch(setIngredientViewList(ingredientList.ingredientList.filter((value) => value.type === 'meat')));
        setBgColor('bg-my-orange');
        break;
      case 'vegetable':
        dispatch(setIngredientViewList(ingredientList.ingredientList.filter((value) => value.type === 'vegetable')));
        setBgColor('bg-my-green');
        break;
      case 'grain':
        dispatch(setIngredientViewList(ingredientList.ingredientList.filter((value) => value.type === 'grain')));
        setBgColor('bg-my-yellow');
        break;
      case 'acc':
        dispatch(setIngredientViewList(ingredientList.ingredientList.filter((value) => value.type === 'acc')));
    ...
    <div className="mt-3 w-[550px]">
      <div className="flex flex-row justify-left">
        {categoryBtn.map((item, idx) => (
          ...
          >
            <p className="font-bold drop-shadow-lg">{Object.values(item.name)}</p>
          </button>
        ))}
      </div>
      <div className={`${bgColor} h-[345px] py-3 opacity-90 rounded-b-lg rounded-tr-lg drop-shadow-lg`}>
        ...
        >
              ...
              {addBtnState ? (
                <>
                  <input
                    ...
                    onChange={(event) => setAddIngredientInput(event.currentTarget.value)}
                  />
          ...
          {searchValue === ''
            ? list.map((item) => <IngredientBtn key={item.name} name={item.name} state={item.state} />)
            : list
                .filter((item) => item.name.includes(searchValue) === true)
                .map((item) => <IngredientBtn key={item.name} name={item.name} state={item.state} />)}
