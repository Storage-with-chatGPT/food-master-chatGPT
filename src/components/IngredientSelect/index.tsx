...
const IngredientSelect = () => {
  ...
  const handleOnClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const { data }: AxiosResponse<DataResponse> = await QuetionChat(quetion, name);
    ...
    <div className="flex flex-row h-full ">
      <div className="flex flex-col w-[500px] items-center justify-center drop-shadow-lg">
        ...
        <button
          onClick={handleOnClick}
          className={`${
            disabledBtnState ? 'bg-gray-100 text-gray-200' : 'bg-white hover:text-white hover:bg-blue-200 text-gray-500'
      ...
      {disabledBtnState ? (
        <div className="absolute w-[550px] h-[705px] flex justify-center items-center   bg-slate-300 bg-opacity-70 z-10  -ml-2 -mt-2">
          <div className="w-[200px] h-[30px] text-center flex flex-row flex-wrap">
            ...
            <p className="text-[10px]">30초에서 1분의 시간이 걸릴 수 있습니다.</p>
          </div>
        </div>
      ) : (
        ''
